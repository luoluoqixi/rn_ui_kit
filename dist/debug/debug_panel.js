...
import { NativeSheet, NativeSheetStack, isIos26Plus, nativeStackStatusBarOptions, useAppBackgroundColors, useColorSchemeSettings, withNativeStackGestureOptions, RN_UI_KIT_PACKAGE_NAME, RN_UI_KIT_PACKAGE_VERSION, } from "rn-ui-kit/core";
...
export function RnUiKitDebugPanel({ backButtonLabel, defaultOpen = true, initialRouteKey = "components", navigationMode = "independent", onOpenChange, open: openProp, pages: pagesProp, sheetMode = false, ...props }) {
    ...
    if (navigationMode === "host") {
        return (_jsx(RnUiKitDebugHostPanel, { backButtonLabel: backButtonLabel, pages: pages, ...props }));
    }
    return (_jsx(RnUiKitDebugPanelContent, { initialRouteKey: initialRouteKey, pages: pages, ...props }));
}
function RnUiKitDebugHostPanel({ backButtonLabel, pages, ...props }) {
    ...
    if (exampleKey != null) {
        ...
        content = (_jsx(RnUiKitDebugHostHomePage, { onOpenSection: (key) => pushDebugRoute({ section: key }), pages: pages }));
    }
    return (_jsx(YStack, { background: "$background", flex: 1, ...props, children: content }));
}
function RnUiKitDebugHostHomePage({ onOpenSection, pages: pagesProp, ...props }) {
    const pages = getDebugPages(pagesProp);
...
function RnUiKitDebugPanelContent({ initialRouteKey = "components", pages, ...props }) {
    ...
    const [sectionSheetPosition, setSectionSheetPosition] = useState(0);
    const [openSectionSheets, setOpenSectionSheets] = useState(new Set());
    return (_jsxs(YStack, { background: "$background", flex: 1, ...props, children: [_jsx(NavigationContainer, { theme: navigationTheme, children: _jsxs(Stack.Navigator, { id: "rn-ui-kit-debug-stack", initialRouteName: "index", screenOptions: debugStackScreenOptions, children: [_jsx(Stack.Screen, { name: "index", options: { title: `${RN_UI_KIT_PACKAGE_NAME} - ${RN_UI_KIT_PACKAGE_VERSION}` }, children: () => (_jsx(RnUiKitDebugHomeRoute, { onOpenInSheet: (key) => setOpenSectionSheets((current) => new Set(current).add(key)), onOpenPanelSheet: () => setPanelSheetOpen(true), pages: pages, onOpenSectionsInSheetChange: (enabled) => {
                                    setOpenSectionsInSheet(enabled);
                                    if (!enabled)
                                        setOpenSectionSheets(new Set());
