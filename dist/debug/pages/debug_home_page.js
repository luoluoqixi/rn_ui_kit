...
function SectionSheetPositionSlider({ onPositionChange, position, }) {
    ...
    return (_jsxs(View, { style: styles.detentSliderRow, children: [_jsx(Minimize2, { color: "$color10", size: 18 }), _jsx(View, { style: styles.detentSliderControl, children: _jsx(Slider, { max: 2, min: 0, onValueChange: (value) => {
                        const nextPosition = value[0] ?? draftPositionRef.current;
                        draftPositionRef.current = nextPosition;
                        setDraftPosition(nextPosition);
                    }, onValueChangeFinished: () => onPositionChange?.(draftPositionRef.current), step: 1, value: [draftPosition] }) }), _jsx(Maximize2, { color: "$color10", size: 18 })] }));
}
