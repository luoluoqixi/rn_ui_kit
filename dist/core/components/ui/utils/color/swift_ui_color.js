...
function parseRgbColor(value) {
    const match = value.match(/^rgba?\s*\(\s*(\d+(?:\.\d+)?)%?\s*,\s*(\d+(?:\.\d+)?)%?\s*,\s*(\d+(?:\.\d+)?)%?\s*(?:,\s*([\d.]+))?\s*\)$/i);
    if (!match) {
        return undefined;
    }
...
function parseHslColor(value) {
    const match = value.match(/^hsla?\s*\(\s*(-?\d+(?:\.\d+)?)\s*,\s*(\d+(?:\.\d+)?)%\s*,\s*(\d+(?:\.\d+)?)%\s*(?:,\s*([\d.]+))?\s*\)$/i);
    if (!match) {
        return undefined;
    }
 ...
 */
export function toSwiftUIHexColor(value) {
    if (typeof value === "string") {
        const trimmed = value.trim();
        if (trimmed.length === 0) {
            return undefined;
