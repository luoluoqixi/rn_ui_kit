...
export function NativeSlider(props) {
    const { value, onValueChange, onValueChangeFinished, min, max, step: stepProp, nativeHaptics, } = props;
    ...
    const handleValueChange = (nextValue) => {
        // 四舍五入到最近的 step，避免浮点数
        const stepped = Math.min(safeMax, Math.max(safeMin, Math.round((nextValue - safeMin) / safeStep) * safeStep + safeMin));
    ...
    return (_jsx(View, { style: { height: 48, width: "100%" }, children: _jsx(Host
        ...
        , { 
            ...
            // iOS 26 的系统原生 Slider 在启用 step 后，拖拽到起点/终点时会持续触发控件自带 haptics。
            // https://www.reddit.com/r/SwiftUI/comments/1tru5h4/swiftui_slider_spams_sensory_feedback/
            ignoreSafeArea: "all", style: { flex: 1, width: "100%" }, children: _jsx(ExpoSlider, { value: currentValue, onEditingChanged: (isEditing) => {
                    if (!isEditing) {
                        onValueChangeFinished?.([latestValueRef.current]);
                    }
                }, onValueChange: handleValueChange, min: safeMin, max: safeMax, step: safeStep, modifiers: trackTintColor != null ? [tint(trackTintColor)] : undefined }) }) }));
}
