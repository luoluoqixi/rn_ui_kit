...
function TamaguiSliderRoot(props) {
    const { children, nativeHaptics, nativeHapticsInterval, onValueChange, onValueChangeFinished, onSlideEnd, thumbCount, thumbProps, trackActiveProps, trackProps, orientation = "horizontal", size = "$4", ...rootProps } = props;
    ...
    return (_jsx(TamaguiSliderPrimitive, { ...rootProps, onSlideEnd: (event, value) => {
            onSlideEnd?.(event, value);
            onValueChangeFinished?.([value]);
        }, onValueChange: handleValueChange, orientation: orientation, size: size, children: children ?? (_jsxs(_Fragment, { children: [_jsx(TamaguiSliderPrimitive.Track, { ...trackProps, children: _jsx(TamaguiSliderPrimitive.TrackActive, { ...trackActiveProps }) }), Array.from({ length: resolvedThumbCount }).map((_, index) => (_createElement(TamaguiSliderPrimitive.Thumb, { size: 30, ...thumbProps, circular: thumbProps?.circular ?? true, index: index, key: index })))] })) }));
}
