...
import { getSliderHapticsBuckets, toARGB, triggerSliderNativeHaptics, useResolvedNativeHaptics, } from "../utils";
export function NativeSlider(props) {
    const { value, onValueChange, onValueChangeFinished, min, max, step: stepProp, colors: colorsProp, nativeHaptics, nativeHapticsInterval, } = props;
    ...
    const lastHapticsBucketsRef = React.useRef(getSliderHapticsBuckets([currentValue], {
    ...
    const resolvedSteps = safeStep > 0 ? Math.max(0, Math.round((safeMax - safeMin) / safeStep) - 1) : 0;
    ...
    return (_jsx(Host, { style: { height: 48, width: "100%", justifyContent: "center" }, children: _jsx(ExpoSlider, { value: currentValue, onValueChange: handleValueChange, onValueChangeFinished: () => onValueChangeFinished?.([latestValueRef.current]), min: safeMin, max: safeMax, steps: resolvedSteps, colors: resolvedColors }) }));
}
