...
function SliderRoot(props) {
    const { children, native = DEFAULT_NATIVE, nativeHaptics, nativeHapticsInterval, onValueChange, onValueChangeFinished, onSlideEnd, thumbCount, thumbProps, trackActiveProps, trackProps, orientation = "horizontal", size = "$4", ...rootProps } = props;
    if (!web && native) {
    ...
    return (_jsx(ReplicaSlider, { ...rootProps, onSlideEnd: (event, value) => {
