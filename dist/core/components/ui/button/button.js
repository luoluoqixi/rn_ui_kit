...
export const Button = forwardRef((props, ref) => {
    ...
    const resolvedTitle = title ??
        ...
        "";
    if (useNativeButton) {
        return (_jsx(RNButton, { accessibilityLabel: props["aria-label"], color: theme.color10?.val ?? theme.color6?.val ?? theme.color?.val, disabled: buttonProps.disabled, onPress: handlePress, title: resolvedTitle }));
    }
    if (isWeb()) {
        return (_jsx(TamaguiButton, { opacity: resolvedOpacity, ...buttonProps, onPress: handlePress, ref: ref, children: children }));
    }
    return (_jsx(TamaguiButtonWithLongPressDelay, { opacity: resolvedOpacity, ...buttonProps, delayLongPress: resolvedDelayLongPress, onPress: handlePress, ref: ref, children: children ?? resolvedTitle }));
});
