...
export function useSheetProviderProps(props, state, options = {}) {
    ...
    const snapPointsProp = props.snapPoints ??
    ...
    const snapPoints = React.useMemo(() => (props.dismissOnSnapToBottom ? [...stableSnapPointsProp, 0] : stableSnapPointsProp), [stableSnapPointsProp, props.dismissOnSnapToBottom]);
    ...
    const scrollBridge = useConstant(() => {
        ...
        const parentDragListeners = new Set();
        const bridge = {
    ...
    const screenSize = snapPointsMode === "percent"
