...
export function isTrueSheetOverlayPortalHost(hostName) {
    return isTrueSheetOverlayPortalHostName(hostName);
}
/**
 * True Sheet：toastLayer 底边补偿（仅 iOS，与 Viewport bottom 分工，勿叠双份）。
 */
export function getTrueSheetOverlayLayoutBottomInset(hostName, safeAreaBottom, detent = 1) {
 ...
 */
export function getTrueSheetCenteredModalLiftAmount(hostName, safeAreaBottom, detent = 1) {
    if (!isTrueSheetOverlayPortalHost(hostName)) {
        return 0;
    }
    const base = safeAreaBottom > 0
        ? safeAreaBottom
        : platform === "android"
            ? ANDROID_TRUE_SHEET_TELEPORT_LAYER_BOTTOM_FALLBACK
            : 0;
    const bottom = platform === "android"
...
export function shouldApplyIosTrueSheetToastLayerInset(hostName) {
    return platform === "ios" && isTrueSheetOverlayPortalHost(hostName);
}
export function getScopedToastViewportBottomInset(viewportName, detent = 1) {
    ...
    if (platform === "ios") {
        if (isTrueSheetOverlayPortalHost(viewportName)) {
            ...
            const inverseCurve = scaleProgress * (1 - scaleProgress) * (1 - scaleProgress) * (1 - scaleProgress);
