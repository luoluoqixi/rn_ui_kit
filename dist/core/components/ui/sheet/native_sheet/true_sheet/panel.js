...
import { getTrueSheetGestureRootStyle, getTrueSheetPanelOverlayLayout, getTrueSheetPanelScrollableProps, mergeTrueSheetContentStyle, shouldUseTrueSheetNativeScrollInsets, } from "./platform_sheet_defaults";
import { resolveTrueSheetGrabber } from "./sheet_chrome";
...
function TrueSheetPanelInner({ backgroundColor, children, chrome = "plain", grabberContentInsetTop, grabber: grabberProp, headerTransparent = false, header: headerProp, name, title, canGoBack = false, onBack, onRequestClose, headerRight, overlayPortalHostName, onDidDismiss, sheetProps, }) {
    ...
    const toolbarHeader = showToolbar && title != null ? (_jsx(TrueSheetToolbarHeader, { canGoBack: canGoBack, headerRight: headerRight, onBack: onBack, title: title, transparent: headerTransparent })) : undefined;
    const resolvedHeader = headerProp ?? toolbarHeader;
    const resolvedGrabberContentInsetTop = grabberContentInsetTop ?? 0;
    const shouldReserveGrabberContentInset = grabber && resolvedHeader == null && resolvedGrabberContentInsetTop > 0;
    const resolvedGrabberOptions = grabber && resolvedHeader == null
        ? {
    ...
    const overlayBody = (_jsx(ScreenOverlayPortalProvider, { hostName: resolvedOverlayPortalHostName, overlayLayout: getTrueSheetPanelOverlayLayout(), children: children }));
    const sheetBody = (_jsx(TrueSheetScrollableBindingProvider, { value: scrollableBinding.providerValue, children: _jsx(TrueSheetScrollLayoutProvider, { insetAdjustment: insetAdjustment, nativeScrollInsetsApplied: shouldUseTrueSheetNativeScrollInsets(sheetScrollable), presentationActive: presented, children: _jsx(GestureHandlerRootView, { style: [
                    styles.gestureRoot,
                    backgroundStyle,
    ...
    return (_jsx(TrueSheet, { ref: scrollableBinding.setSheetRef, backgroundColor: backgroundColor, header: resolvedHeader, name: name, ...defaultSheetProps, ...sheetProps, grabber: grabber, grabberOptions: resolvedGrabberOptions, onDetentChange: overlayLayoutSync.onDetentChange, onDidDismiss: handleDidDismiss, onDidPresent: handleDidPresent, onDragChange: overlayLayoutSync.onDragChange, onDragEnd: overlayLayoutSync.onDragEnd, onPositionChange: overlayLayoutSync.onPositionChange, onWillPresent: overlayLayoutSync.onWillPresent, style: mergeTrueSheetContentStyle(mergedScrollable, [sheetProps?.style, backgroundStyle]), children: sheetBody }));
}
/**
 ...
 */
export function TrueSheetPanel(props) {
    return (_jsx(TrueSheetOverlayLayoutProvider, { children: _jsx(TrueSheetPanelInner, { ...props }) }));
}
