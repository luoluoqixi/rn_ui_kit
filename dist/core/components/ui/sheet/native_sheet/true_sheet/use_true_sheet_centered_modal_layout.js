...
import { useTrueSheetOverlayDetent, useTrueSheetOverlaySheetTopPosition, } from "./overlay_layout_context";
...
import { getTrueSheetCenteredModalLiftAmount, isTrueSheetOverlayPortalHost, } from "./overlay_toast_layout";
/**
 * True Sheet 内居中 Dialog / AlertDialog：上移内容，不缩短 teleport 层（避免遮罩底部漏光）。
 * 视觉位移约 liftAmount / 2。
 */
export function getTrueSheetCenteredModalContentOffsetY(hostName, safeAreaBottom, detent = 1, sheetTopPosition = null) {
    const insetLift = -Math.round(getTrueSheetCenteredModalLiftAmount(hostName, safeAreaBottom, detent) / 2);
    const detentOffset = getTrueSheetCenteredModalDetentOffsetY(sheetTopPosition, detent);
    return insetLift + detentOffset;
}
export function useTrueSheetCenteredModalContentOffsetY() {
