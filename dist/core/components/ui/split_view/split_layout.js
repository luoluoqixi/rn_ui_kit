...
import { SplitViewModel } from "./split_view_model";
...
const getWebClassNameProps = (className) => IS_WEB && className ? { className } : {};
...
const getPointerCoordinate = (event, vertical) => {
    return vertical ? event.clientY : event.clientX;
};
const bindDocumentPointerDrag = (onMove, onEnd) => {
...
const resolveMobileHandlePosition = (vertical, position) => {
    if (vertical) {
...
const getMobileHandleStyle = (vertical, position, offset) => {
...
const getMobileHandlePositionForSash = (index, fallbackPosition, positions) => {
    return positions?.[index] ?? fallbackPosition;
};
const getMobileHandleOffsetForSash = (index, fallbackOffset, offsets) => {
    return offsets?.[index] ?? fallbackOffset;
};
const SplitLayoutInner = forwardRef(({ children, className, defaultSizes, maxSize = Number.POSITIVE_INFINITY, minSize = 30, mobileHandleOffset = 0, mobileHandlePosition = "center", mobileHandleOffsets, mobileHandlePositions, onChange, onDragEnd, onDragStart, onReset, onStateChange, onVisibleChange, proportionalLayout = true, separator = true, snap = false, storageKey, style, vertical = false, }, ref) => {
    const layoutServiceRef = useRef(new LayoutService());
    const modelRef = useRef(null);
    const initialLayoutStateRef = useRef(null);
    const initialStoredStateHydratedRef = useRef(false);
    const initialStoredStateRef = useRef(undefined);
    const nativePendingDoubleTapRef = useRef(null);
    const nativeLastTapRef = useRef(null);
    const webLastTapRef = useRef(null);
    const webPendingDoubleTapRef = useRef(null);
    const panesRef = useRef([]);
    const stableDefaultSizesRef = useRef(undefined);
    ...
    const webDragCleanupRef = useRef(null);
    const webDraggingRef = useRef(false);
    const [activeSashIndex, setActiveSashIndex] = useState(null);
    const [dragging, setDragging] = useState(false);
    const [hoveredSashIndex, setHoveredSashIndex] = useState(null);
    ...
    stableDefaultSizesRef.current = stabilizeNumberArray(stableDefaultSizesRef.current, defaultSizes);
    const stableDefaultSizes = stableDefaultSizesRef.current;
    const theme = useTheme();
    const separatorColor = useSeparatorColor();
    const sashActiveColor = resolveThemeColor(theme.accent10 ?? theme.accent8 ?? theme.borderColorHover ?? theme.borderColor, FALLBACK_SASH_ACTIVE_COLOR);
    const { ready: storageReady, state: storedState, setState: setStoredState, } = useSplitLayoutStorage(storageKey);
    useEffect(() => {
    ...
    const panes = useMemo(() => normalizePanes(children, minSize, maxSize, snap), [children, maxSize, minSize, snap]);
    const paneConfigKey = useMemo(() => getPaneConfigKey(panes), [panes]);
    panesRef.current = panes;
    const emitState = useCallback((model) => {
    ...
    const createInitialLayoutState = useCallback(() => {
        ...
        if (layoutSize <= 0 || currentPanes.length === 0)
            return null;
        ...
        const descriptorViews = currentPanes.map((pane, index) => {
            ...
            const visibleSize = clamp(defaultSize ?? preferredSize, pane.minSize, Math.min(pane.maxSize, layoutSize));
        ...
        const descriptor = {
            size: descriptorViews.reduce((sum, item) => sum + (typeof item.size === "number" ? item.size : 0), 0),
    ...
    const createModel = useCallback(() => {
        const currentPanes = panesRef.current;
        if (!storageReady || layoutSize <= 0 || currentPanes.length === 0)
            return null;
        ...
        const canUseStoredState = sourceState?.sizes.length === currentPanes.length &&
        ...
        const descriptorViews = currentPanes.map((pane, index) => {
            ...
            const defaultSize = canUseDefaultSizes ? stableDefaultSizes[index] : undefined;
            const visibleSize = clamp(storedSize ?? defaultSize ?? preferredSize, pane.minSize, Math.min(pane.maxSize, layoutSize));
            ...
            if (initialLayoutState) {
                ...
                panes.forEach((pane, index) => {
                    ...
                    if (preferredSize !== undefined)
                        model.resizeView(index, preferredSize);
                });
            }
            persistState(emitState(model));
        },
    ...
    const sashBoundaries = useMemo(() => panes.slice(0, -1).map((_, index) => {
        return PixelRatio.roundToNearestPixel((offsets[index] ?? 0) + (sizes[index] ?? 0));
    }), [offsets, panes, sizes]);
    const resetSash = useCallback((index) => {
        const model = modelRef.current;
        const initialLayoutState = initialLayoutStateRef.current;
        if (!model || !initialLayoutState)
            return;
        webDragCleanupRef.current?.();
        webDragCleanupRef.current = null;
    ...
    const moveDrag = useCallback((delta) => {
        const model = modelRef.current;
        if (!model)
            return;
    ...
    const bindWebPointerTracking = useCallback((index, startPointer) => {
    ...
    const startWebSashDrag = useCallback((index) => (event) => {
        if (!IS_WEB)
            return;
    ...
    const startNativeSashDrag = useCallback((index) => {
        const now = Date.now();
    ...
    const moveNativeSashDrag = useCallback((index, movement) => {
        if (nativePendingDoubleTapRef.current === index &&
            Math.abs(movement) > SASH_TAP_MOVE_TOLERANCE) {
            nativePendingDoubleTapRef.current = null;
        }
        moveDrag(movement);
    }, [moveDrag]);
    const finalizeNativeSashDrag = useCallback((index, movement, succeeded) => {
    ...
    const sashNativeGestures = useMemo(() => {
        if (IS_WEB)
            return [];
        return Array.from({ length: Math.max(panes.length - 1, 0) }, (_, index) => Gesture.Pan()
            .minDistance(0)
    ...
    return (_jsx(View, { ...getWebClassNameProps(className), onLayout: handleLayout, style: [styles.root, styles.webRoot, style], children: _jsxs(View, { style: styles.container, children: [panes.map((pane, index) => {
                    const paneSize = sizes[index] ?? 0;
                    const paneOffset = offsets[index] ?? 0;
                    const paneStyle = vertical
                        ? { top: paneOffset, height: paneSize, left: 0, right: 0 }
                        : { left: paneOffset, width: paneSize, top: 0, bottom: 0 };
                    return (_jsx(View, { ...getWebClassNameProps(pane.className), pointerEvents: visible[index] ? undefined : "none", style: [styles.pane, paneStyle, pane.style], children: pane.children }, pane.key));
                    ...
                    if (!canDragSash && !showSeparatorLine)
                        return null;
                    const sashSize = IS_MOBILE ? MOBILE_SASH_SIZE : DEFAULT_SASH_SIZE;
                    const showMobileHandle = canDragSash && IS_MOBILE && (visible[index] ?? true) && (visible[index + 1] ?? true);
                    const mobileHandleStyle = getMobileHandleStyle(vertical, getMobileHandlePositionForSash(index, mobileHandlePosition, mobileHandlePositions), getMobileHandleOffsetForSash(index, mobileHandleOffset, mobileHandleOffsets));
                    ...
                    const sashView = (_jsxs(View, { pointerEvents: canDragSash ? undefined : "none", style: [
                            styles.sash,
                            ...
                            : undefined), children: [_jsx(View, { pointerEvents: "none", style: [
                                    styles.sashLine,
                                ...
                                ] }), showMobileHandle ? (_jsx(View, { pointerEvents: "none", style: [
                    ...
                    return nativeGesture ? (_jsx(GestureDetector, { gesture: nativeGesture, children: sashView }, sashKey)) : (sashView);
                })] }) }));
});
const SplitLayoutPane = forwardRef(({ className, children, style }, ref) => {
    return (_jsx(View, { ref: ref, ...getWebClassNameProps(className), style: style, children: children }));
});
SplitLayoutPane.displayName = "SplitLayout.Pane";
const resolvePreferredSize = (preferredSize, layoutSize) => {
...
const isPaneElement = (child) => {
    return child.type.displayName === SplitLayoutPane.displayName;
};
const normalizePanes = (children, parentMinSize, parentMaxSize, parentSnap) => {
...
const canRenderSash = (panes, index) => {
...
export const SplitLayout = Object.assign(forwardRef(({ storageAdapter, storageFallbackState, storageKey, ...props }, ref) => {
    if (storageAdapter || storageKey || storageFallbackState) {
        return (_jsx(SplitLayoutProvider, { fallbackState: storageFallbackState, storageAdapter: storageAdapter, storageKey: storageKey, children: _jsx(SplitLayoutInner, { ref: ref, ...props }) }));
    }
    return _jsx(SplitLayoutInner, { ref: ref, ...props });
